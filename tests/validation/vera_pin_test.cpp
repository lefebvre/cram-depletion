...
ReactionType reactionFromName(const std::string& s, bool& known) {
  known = true;
  if (s == "fission")
...
struct VeraCase {
  ...
  std::map<std::pair<int, int>, std::vector<ChainReaction>> branches;  // (parent,rxn)->branches
  std::vector<double> dt;                                              // per step (s)
  std::vector<double> flux;                                            // per step (n/cm^2/s)
  std::map<std::int64_t, std::vector<ReactionXS>> xs;  // fixed: parentKey -> reactions
...
TEST_P(VeraPin, ReproducesOpenMCPredictorTrajectory) {
  ...
  VeraCase c;
  auto topo = loadDepletionChainXml(c.chain, (root / "chain.xml").string());
  // (parentKey, reaction) -> the chain's branch(es) for that reaction. A
  // reaction absent here is not in the chain for that nuclide and must be
  // ignored (OpenMC only applies reactions the chain defines), even if the
  // micro-xs file carries a tally for it.
  for (const auto& r : topo)
    c.branches[{r.parent.key(), static_cast<int>(r.type)}].push_back(r);
  ...
  // reactions the chain defines for the nuclide are applied; a (n,gamma) that
  // branches to ground + metastable splits the cross section by branch fraction.
  for (const auto& row : readCsv(root / "micro_xs.csv")) {
    Zai parent;
    if (!parseNuclideName(row[0], parent))
      continue;
    bool known = false;
    ReactionType type = reactionFromName(row[1], known);
    if (!known)
      continue;
    auto it = c.branches.find({parent.key(), static_cast<int>(type)});
    ...
    for (const ChainReaction& br : it->second) {
      ReactionXS rx;
      rx.type = type;
      rx.sigma = sigma * br.branching;
      rx.target = br.hasTarget ? br.target : Zai{};  // {0,0,0} -> removal only
      c.xs[parent.key()].push_back(rx);
    }
  }

  // density.csv: step,nuclide,atoms  (step 0..N)
  c.refDensity.assign(c.steps + 1, Eigen::VectorXd::Zero(c.chain.size()));
  for (const auto& row : readCsv(root / "density.csv")) {
    const int step = std::stoi(row[0]);
    Zai z;
    if (!parseNuclideName(row[1], z) || step < 0 || step > c.steps)
      continue;
    const int idx = c.chain.indexOf(z);
    if (idx >= 0)
      c.refDensity[step](idx) = std::stod(row[2]);
  }
  ...
  for (const auto& [parentKey, rxns] : c.xs) {
    Zai parent{static_cast<int>(parentKey / 10000), static_cast<int>((parentKey / 10) % 1000),
               static_cast<int>(parentKey % 10)};
  ...
  for (int k = 1; k <= c.steps; ++k) {
    sys.setConstantFlux(c.flux[k - 1]);
    n = integ->step(n, c.dt[k - 1]);
  }
  ...
  for (const char* name : kImportant) {
    Zai z;
    if (!parseNuclideName(name, z))
      continue;
    const int i = c.chain.indexOf(z);
    if (i < 0)
      continue;
    const double ref = c.refDensity[c.steps](i);
    if (ref <= 0.0)
      continue;
    ++checked;
    EXPECT_LT(std::abs(n(i) - ref) / ref, 1e-3) << name << ": cram=" << n(i) << " openmc=" << ref;
  }
